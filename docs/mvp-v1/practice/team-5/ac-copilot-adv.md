...
建議（方向性 / 驗證方向）：
- 建議驗證來源：rawData 的欄位（例如 Status、Story Points）或後端聚合 API；若無 assignee，可允許使用 TaskTags 或 Projects 作為 fallback。
- 計算方向保留彈性：在驗證時確認卡片顯示的數字和後端匯總一致即可，狀態到分類的映射可依 `table-schema` 定義。
...
建議（方向性 / 驗證方向）：
- 驗證重點為「存在未指派匯總卡片」且該卡片數字等於後端或 rawData 中未指派項目的匯總；提示文案可留給 UX 決定。
...
建議（方向性 / 驗證方向）：
- 驗證可檢查：錯誤訊息是否顯示、是否有快取資料回退顯示，且 UI 清楚標示該資料為快取版本。
...
建議（方向性 / 驗證方向）：
- 建議驗證：數量總和應等於總件數；百分比為 count/total 的表示（顯示精度可由 UI 決定，如小數點 1 位）。
- 狀態清單應以 `table-schema` 為準，Unknown 或其他值可聚合顯示為「Other」。
...
建議（方向性 / 驗證方向）：
- 驗證重點為「所有預期狀態均可見」與「最高占比狀態有視覺提示」。是否顯示提示文字或圖示留給設計決定。
...
建議（方向性 / 驗證方向）：
- Completion Rate 的計算可依 story-points 或 issue-count；在 AC 保留彈性，驗證時確認 UI 與後端使用相同計算方式。
- HealthStatus 可為可配置策略的輸出（例如依百分比區間判定），驗證重點為 UI 有明確顯示與說明而非特定閾值。
...
建議（方向性 / 驗證方向）：
- 驗證可以檢查當 Story Points 全為空時，UI 顯示替代指標並隱藏燃盡圖；替代指標的選擇留給團隊決定。
...
建議（方向性 / 驗證方向）：
- 驗證重點為「UI 能明確區分不可用的燃盡圖」與「提供重新取得資料的操作」。錯誤紀錄的具體格式留給工程團隊決定。
